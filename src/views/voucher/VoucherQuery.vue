  ...
  <div>
    ...
    <div style="margin-bottom: 16px">
      <a-space>
        ...
        <!-- 匹配选择框 -->
        <a-select
          v-model:value="selectedFlowStatus"
          placeholder="match"
          style="width: 200px"
...
import { GoOn, getUsrOffice, getVoucherType } from "@/http";
...
const selectedFlowStatus = ref<string>("");
...
const FlowStatusOptions = ref([
  { label: "全部", value: "" },
  { label: "流程中", value: "onflow" },
  { label: "已结案", value: "finish" },
...
const loadOptions = async () => {
  try {
    ...
    // 获取办公室选项
    const officeRes = await getUsrOffice();
...
const buildRequestParams = () => {
  return {
    ...
    Flow_Status: selectedFlowStatus.value || "",
...
const preview = async (record: DataItem) => {
  try {
    ...
    message.success("文件下载开始,请点击浏览器下载路径查看");
  } catch (error: any) {
